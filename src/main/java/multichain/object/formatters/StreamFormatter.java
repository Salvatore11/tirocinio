...
public class StreamFormatter {
	...
	public final static StreamKeyTxid formatStreamKeyTxid(Object objectStreamKeyTxid) {
		StreamKeyTxid streamKeyTxid = new StreamKeyTxid();

		if (objectStreamKeyTxid != null && LinkedTreeMap.class.isInstance(objectStreamKeyTxid)) {
			GsonBuilder builder = new GsonBuilder();
			Gson gson = builder.create();

			String jsonValue = gson.toJson(objectStreamKeyTxid);
			streamKeyTxid = gson.fromJson(jsonValue, StreamKeyTxid.class); // StreamKeyItem.class
		}
	...
	public final static StreamKeyItem formatStreamKeyItem(Object objectStreamKeyItem) {
		...
		if (objectStreamKeyItem != null && LinkedTreeMap.class.isInstance(objectStreamKeyItem)) {
			...
			//System.out.println("IN STREAM FORMATTER, JsonVALUE: " + jsonValue);
	...
	public final static List<StreamKeyTxid> formatStreamKeyTxid1(List<Object> objectStreamKeyTxid1) {
		List<StreamKeyTxid> streamKeyTxid = new ArrayList<StreamKeyTxid>();

		if (objectStreamKeyTxid1 != null) {
			for (Object objectStreamKeyTxid : objectStreamKeyTxid1) {
				streamKeyTxid.add(formatStreamKeyTxid(objectStreamKeyTxid));
			}
		}
